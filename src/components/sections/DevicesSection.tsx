...
export function DevicesSection() {
  ...
  // Find the active color, default to a subtle deep purple/blue if none hovered
  const activeColor = devices.find(d => d.id === hoveredDevice)?.color || 'rgba(168, 85, 247, 0.15)';

  return (
    <section 
      className={styles.devicesSection}
      style={{ '--active-glow': activeColor } as React.CSSProperties}
    >
      <div className={styles.ambientBackground} />
      
      <div className={styles.container}>
        <div className={styles.header}>
          <span className={styles.pill}>Stream Anywhere</span>
          <h2 className={styles.title}>
            Available on <span className="text-gradient">Every Device</span>
          </h2>
        ...
        <div className={styles.grid}>
          {devices.map((device) => (
            <div 
              key={device.id} 
              className={`${styles.deviceCard} ${hoveredDevice && hoveredDevice !== device.id ? styles.dimmed : ''}`}
